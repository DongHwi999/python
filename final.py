...
for filename in glob(train_path1 + "*"):
    for img in glob(filename + "/*.jpg"):
        an_img = PIL.Image.open(img).resize((int(32), int(32)))  # read img
...
# training_labels = tf.one_hot(training_labels, 1)
...
image_generator = ImageDataGenerator(
    ...
    vertical_flip=False #  x축을 기준으로 반전 (위 아래 뒤집기)
)

augment_size = 100000 # 데이터를 추가할 개수, 총 108228개로 증가함
